...
int scene_run(Player cha) {
    ...
    if (scene_process(event,cha) == MSG_TERMINATE) {
        return MSG_TERMINATE;
    }
...
public:
    ...
    /*int site(){
        if(scene_state == SCENE_MENU) return 0;
        else if(msg_state == MSG_CHANGE_SETTING) return 1;
        else if(msg_state == MSG_TERMINATE) return 2;
        else if(msg_state == MSG_ABOUT) return 3;
        else if(msg_state==MSG_BACK_TO_MENU) return 4;
        else if(msg_state== MSG_GAME_OVER) return 5;
        else if(msg_state==MSG_GAME_RESTART) return 6;
    }*/
...
int scene_process(ALLEGRO_EVENT event,Player cha) {
    int msg;
    //Player cha;
    if (scene_state == SCENE_MENU) {
        ...
        if (msg == MSG_GAME_START) {
            ...
            //now_state.State_init(cha);
            now_state.destructor();
